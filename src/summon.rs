...
//
// 常駐をフローティングに変えるわけではない（決定202607302257は維持）。タイルは幅を返す
...
impl State {
    ...
    pub(crate) fn summon_floating_if_absent(&mut self) {
        ...
        //（決定202608011644で一度実際に壊した経路）の方がコストが高い
        ...
        //（トグル・決定202608011644）。召喚された本人はこの pipe を受け取れないので、
        // 出した側が始末をつけるしかない。
        //
        ...
        // 決定202608012141の同期は「可視インスタンスが PaneUpdate で新入りに気づいて配る」
    ...
    // 取り残された召喚インスタンスを閉じる（決定202608011644の掃除の逃げ道）。判定材料は
    // 「同じURLのプラグイン」かつ「フローティング」だけに絞る（常駐サイドバーは
    // タイル・決定202607302257）。召喚側が発行したIDを覚えておく手もあるが、覚えている
