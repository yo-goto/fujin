...
//
// 専用サブモードは作らない — マーク（決定39）と同じトグルだけの横断的操作で、
...
impl State {
    ...
    //
    // **番号ジャンプサブモード中は更新しない**（決定42）。候補を絞っている
    // あいだは対象ペインが定まらず（1件に確定した瞬間にジャンプするので選ぶ間が
    // 無い。決定29）、直前の表示を維持するほうが筋が通る
    ...
    // navモードの外にプレビューだけ残すのは決定34（フォーカスの預かり）と
    // 整合しない
    ...
    // プレビュー中のペインを既読にする（決定42）。プレビューはフォーカスも
    ...
    // 指定ペインを既読にして兄弟インスタンスへ配る（決定10・決定13）。
    // 注意を引く状態を持っていなければ何も起きない。
    //
    // コマンド状態は猶予（決定32の `awaiting_refocus`）を見ない `force_read` を
    ...
    //
    // 臨時召喚（決定16）と同じく自分のURLを configuration 付きで開く。
    // 開いた側（＝権威インスタンス）にはフォーカスが残らない（実測では
    // 新しく開いたペインへ移る）ので、預かったフォーカス（決定34）を
    // 取り返しておく
    fn open_preview_pane(&mut self) {
        ...
        // 決定34）。**退場の判定はこの取り返しを待たない** — `refresh_focus` は
    ...
    //
    // `get_full_scrollback` は使わない（決定42）。欲しいのは「いま画面に見えて
    // いる範囲」で、遡れる全部ではない
    ...
    // フォーカスの判定（決定34）が自分の一部として扱うために要る
