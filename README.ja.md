...
| `/` | 検索サブモードに入る（下記） |
| `n` | 番号ジャンプサブモードに入る（下記） |
| `p` | トリアージモードに入る（対応が必要なペインを緊急度順に一覧） |
| `?` | キーのヘルプを開く（下記） |
| `Esc` / `q` | モードを抜ける |
...
navモード中は、作業していたペインを囲む zellij の枠も一時的に消えます。navモードは
実フォーカスを動かさないので、そのままではフォーカス中を示す枠とサイドバーの
ハイライトが同時に見えて、どちらが操作対象か分からなくなるためです。枠
（とタイトルバー）はモードを抜けると戻ります。元から枠が無いペイン
（レイアウトで `borderless=true` にしている場合や `pane_frames false` の環境）は
そのままです。
...
ヘッダの `?:help` / `esc:exit` だけに絞ってあります。`?` を押すとヘッダとフッタは
...
plugins {
    fujin location="file:~/.config/zellij/plugins/fujin.wasm" {
        ...
        // 非フォーカス時にサイドバーのフッタへ出すキー表記。
        // fujin_up / fujin_down / fujin_go に割り当てたキーをそのまま書きます
        up_key   "Alt u"
        down_key "Alt d"
        go_key   "Alt g"
    }
