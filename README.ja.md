...
<div align="center">
  <img src="assets/demo.png" width="100%"
       alt="3つのzellijタブにまたがる3体のClaude Codeエージェントを並べたfujinのサイドバー。各行に状態アイコンと作業ディレクトリが出ており、右にフォーカス中のエージェントのペインが見えている。" />
</div>
...
zellij 0.44 以上と `curl`、それに **`jq`** が要ります（`jq` はフックが使うので、
エージェントの状態を出すなら必須です。macOS 13 以降は標準で入っています）。
**Rust は要りません。** `jq` を入れない場合は `--no-hooks` を付けてください——
一覧・ジャンプ・検索は動き、状態表示だけが出なくなります。
...
ターミナル）で動く Claude Code、サイドバーが起動していないとき、`jq` が無いときは、
いずれも完全な no-op です（エラーも出ません）。
