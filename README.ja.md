...
開いたペイン）にも、フックの設定なしで同じアイコンが出ます。走っているあいだが
`working`、終了コード 0 で終われば `done`、それ以外（非0・シグナル・中断）は `error`
です——長いビルドが終わったことが、エージェントと同じ形で分かります。zellij 自身が
コマンドを知っているので設定は要りません。`blocked` / `idle` は持たず、同じペインに
エージェントが登録されている場合はエージェント側の状態が優先されます。

ペイン名の後ろにはカウンタが付くことがあります。`+N` が稼働中のサブエージェント数、
`[N]` が未完了タスク数です。フローティングペインは名前が `(name)` と丸括弧で
囲まれます。ペイン名が空のペインは、コマンドペインならコマンド文字列、
そうでなければ cwd を代わりに出します。

`done` / `blocked` / `error` は**そのペインにフォーカスすると自動的にクリア**されます（既読モデル）。未対応のものだけが光ります。
通り過ぎただけでは消えません——フォーカスしてしばらく留まって初めて既読になるので、
別のペインを経由してジャンプしても途中の状態は消えません。

一覧が画面の高さに収まらないときは、選択行が見えるところまで自動でスクロールします。
隠れている行は上下端の `▴ … N more` / `▾ … N more` が示します。
...
フォーカス中のタブにサイドバーが1つも居ないときは、`Ctrl+y` を押すと fujin が
自分をフローティングで一時的に呼び出します。通常は `Esc` やジャンプで自分から
閉じますが、何かの拍子に取り残されると、フォーカス巡回から外れているため
ユーザーからも CLI からも閉じられません。次のコマンドで掃除できます。
...
`▲ fujin  [nav]` に変わり、フッターに `?:help  esc:exit` が出て、以下のキーが
効きます:
...
navモード中に `/` を押すとフッターがクエリ入力行（`/…`）に変わり、ペイン名・
...
上記以外のキー（`←` / `→` / ファンクションキーなど）を押すと、navモードごと抜けます
（navモード本体と同じ安全弁です）。

操作状態のキー（**下記以外の無修飾キーは何も起きません**）:
...
両方の状態で使えるキーが2つあります。印字可能文字がクエリに取られるため、
`Alt` を付けた形で通しています。

| キー | 動作 |
|---|---|
| `alt+m` | カーソル位置のペインのマークを付け外し（下記） |
| `alt+p` | プレビューの表示を切り替える（下記） |

この2つを除き、`Ctrl` / `Alt` / `Super` を伴うキーはどちらの状態でもnavモードごと
抜けます。

クエリは検索を抜けるたびに破棄され、次回は常に空から始まります。cwd はフック
から通知を受けたペインだけが持つので、一般のシェルペインは cwd では一致しません。
一致が0件のときは、一覧の代わりに `no matches` と出ます。
...
番号列はサブモード中だけ出るので、平常時の行の幅配分は変わりません。フッターは
`n` に続けて入力中の数字を表示します。まだ候補に残っている番号だけがキーの色で
点り、外れた番号は落ちて表示されます。

### トリアージ（navモード内の `t`）

`t` を押すとヘッダが `▲ fujin  [tri]` に変わり、一覧がタブの壁を無視したフラット
表示に切り替わります。状態を持つペイン（`error` / `blocked` / `working` /
`done`）だけが緊急度順（`error` > `blocked` > `working` > `done`。同じ階層内は
直近に状態が変わった順）に並び、行の右端に所属タブ名が付きます。状態を一度も
通知していないペイン・既読済み（`idle`）のペインは一覧に出ません。エージェントを
載せていないコマンドペインの状態も、同じ優先度階層に混ぜて並びます。

| キー | 動作 |
|---|---|
| `j` / `↓` / `Tab`、`k` / `↑` / `Shift+Tab` | カーソル移動 |
| `g` / `G` | 先頭 / 末尾 |
| `Enter` | 選択中のペインへジャンプしてモードを抜ける（一覧が空のときは何もしない） |
| `m` / `M` | マークを付け外し / すべて外す |
| `p` | プレビューの表示を切り替える |
| `r` | プレビュー中のペインを既読にする |
| `?` | キーのヘルプを開く（下記） |
| `Esc` | トリアージ一覧を抜けて通常のツリー表示へ戻る（navモードは継続） |

`Esc` で戻ると、トリアージに入る前に選んでいた行へ選択が戻ります。フッターの
ヒントも `?:help  esc:back` と、退場ではなくツリー表示へ戻る意味になります。
一覧に出すものが1つも無いときは `nothing to triage` と出ます。

### マーク（navモード内の `m` / `M`）

`m` を押すと、いま光っている行のペインにマークが付きます。もう一度押すと外れ、
`M` ですべて外せます。マークされた行には状態アイコンの左に `✓` が出ます
（1つでもマークがあるフレームだけ列が出るので、平常時の行の幅配分は変わりません）。

- ツリー表示・トリアージ一覧では `m` / `M`、検索の絞り込み結果では `alt+m` です
- **タブをまたいでマークできます。** 別のタブの行でも同じように積み上げられます
- **navモードを抜けても保持されます。** 次に入り直したときも付いたままです
- 対象のペインが閉じられると、そのマークだけ自動的に外れます
- 終了操作（下記）を実行するとすべて外れます。`Esc` で取り消した場合は残ります

いまのところマークの使い道は終了操作の一括指定です。

### ペインの終了操作（navモード内の `d`）

`d` を押すとフッターが確認プロンプト（`c:close k:kill x:kill+close`）に変わり、
ヘッダの `▲` ごとテーマのエラー色になります。3つのうちどれかを選ぶまで何も起きず、
`Esc` で取り消せます。プロンプトにペイン名は出しません — 対象は選択行のハイライトで
...
（[マーク](#マークnavモード内の-m--m)を参照）。マークがあるときは
`3 panes  c:close k:kill x:kill+close` のように件数が前置されます——何件消えるかは
行のハイライトからは読み取れないためです。実行するとマークはすべて外れます
（`Esc` で取り消した場合は残ります）。対象が1つも無いときは、そもそもプロンプトが
出ません。
...
フッターの `?:help` / `esc:exit` だけに絞ってあります。`?` を押すとヘッダはそのままに、
ツリーの部分がキー一覧と状態アイコン凡例に切り替わり、フッターは
`press any key to close` に変わります。何かキーを押せば元の表示（navモードまたは
各サブモード）に戻ります。ヘルプを閉じるためのキーはそのままでは操作として解釈
されないので、閉じたあとに改めて押し直してください。navモードは開いている間も
継続します。

キー一覧の中身はそのとき居るモード（ツリー表示・検索・番号ジャンプ・トリアージ・
終了操作）ごとに変わり、状態アイコンの凡例はどこから開いても同じものが出ます。
オーバーレイが画面の高さに収まらないときは、一覧と同じあふれマーカーが出ます
（オーバーレイはどのキーでも閉じるので、スクロール用のキーは持てません）。
...
`_down` / `_go` / `_mode` / `_toggle_cwd`、それに後始末用の `fujin_dismiss` だけです。

- `fujin_toggle_cwd` はペイロード無しなら反転、`true` / `false` を渡せばその値に
  揃えます（複数タブへ同じ向きを押し付けたいときはこちら）
- `fujin_dismiss` は、取り残されたフローティングのサイドバーを閉じます
  （[トラブルシューティング](#フローティングのサイドバーが残ってしまった)を参照）

`fujin_sync_state` / `_read` / `_selection` / `_command` / `_mark` / `_preview` /
`_width` はインスタンス間の同期用の内部プロトコルなので、外から叩かないでください。
