...
要求権限が増えた場合にも起こります。zellij のキャッシュディレクトリにある
`permissions.kdl` から該当 wasm のエントリを削除し、セッションを作り直して
承認し直してください。
...
いまどちらに居るかは3つの手がかりで分かります。

| | 編集状態 | 操作状態 |
|---|---|---|
| 疑似カーソル | バー `▏` | ブロック `▌` |
| クエリの文字色 | 通常 | 沈む（dim） |
| フッターの操作ヒント | `esc:move  enter:jump` | `j/k:move  ?:help  i:edit …` |

編集状態では日本語入力（IME）のために端末の実カーソルを同じ位置に置くので、
**端末のカーソル形状がブロックだと、疑似カーソルのバーがその下に隠れます**。
`vim` と同じ「挿入＝バー／ノーマル＝ブロック」の見え方にしたい場合は、端末側の
カーソルをビームに設定してください（例: alacritty なら `[cursor.style]` の
`shape = "Beam"`）。この設定をしなくても、クエリの文字色とヒントで状態は分かります。
