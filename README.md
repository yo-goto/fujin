...
所属タブ名・cwd に対するファジー検索でツリーを絞り込める。一致した文字は
ハイライトされ、その場所がどのフィールドに当たったかの提示を兼ねる
（cwd ヒット時は `show_cwd` が無効でもその行に cwd が出る）。
...
クエリは検索を抜けるたびに破棄され、次回は常に空から始まる。cwd はフック
（後述）から通知を受けたペインだけが持つ — 一般のシェルペインは cwd では
一致しない。
...
外部クレートは足さず、標準の `#[test]` のみ。例外として、ホスト関数に
依存しない純粋ロジックのモジュール（`src/search.rs`）は、テストも同じ
ファイル内の `#[cfg(test)] mod tests` に置く。
...
検索サブモードのロジックは単体テストで守られているが、**実キーが
`InterceptedKeyPress` として届く配送経路だけはテストできない**。検索まわりを
触ったら、実セッションで navモード → `/` → クエリ入力 → `Enter` ジャンプ、
および fujin の居ないタブ（臨時召喚）での二段階 `Esc`（1回目で検索だけ閉じ、
2回目でフローティングが閉じる）を確認すること。
